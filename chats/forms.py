    ...
    def value_from_datadict(self, data, files, name):
        ...
        for val in values:
            try:
                ...
                val_str.append(val)
        old_tags = list(
            self.queryset.filter(**{"pk__in": val_int}).values_list("pk", flat=True)
        )
...
class InviteForm(forms.Form):
    users = forms.ModelMultipleChoiceField(
        queryset=get_user_model().objects.all(),
        label="Find peoples",
        help_text="Search by username or full name without including @",
        widget=RecipientWidget,
    )
    room = forms.ModelChoiceField(
        queryset=DiscussionRoom.objects.all(), widget=forms.HiddenInput()
    )
